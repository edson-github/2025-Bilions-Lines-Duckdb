...
def construir_lista_estacoes_meteorologicas():
    """
    Obtém os nomes das estações meteorológicas a partir de um arquivo e remove
    duplicatas.
    """
    with open('./data/amostra_44k.csv', 'r', encoding="utf-8") as arquivo:
        return list(set(
            linha.split(';')[0] for linha in arquivo if "#" not in linha
        ))
...
def formatar_tempo_decorrido(segundos):
    ...
    return (f"{int(minutos)}m {int(segundos)}s"
            if minutos else f"{segundos:.2f}s")
    ...
    try:
        with open(arquivo_saida, 'w', encoding="utf-8") as arquivo:
            for _ in range(num_registros // tamanho_lote):
                ...
                template = "{};{:.1f}"
                linhas = '\n'.join(
                    template.format(est, random.uniform(-99.9, 99.9))
                    for est in lote
                )
        ...
        print(f"Tempo decorrido: {formatar_tempo_decorrido(tempo_final)}")
